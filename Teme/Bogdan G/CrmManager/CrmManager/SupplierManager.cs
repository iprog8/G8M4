...
namespace CrmManager.Managers
{
    public class SupplierManager
    {
        ...
        {
            CRMEntities db = new CRMEntities();
            Supplier toBeDeleted = db.Supplier.Find(id);
            if (toBeDeleted == null) return;
            db.Supplier.Remove(toBeDeleted);
            db.SaveChanges();
        }
        public bool VerifyId(string id)
        {
            if (int.TryParse(id, out int newId))
            {
                ...
                else if (new CRMEntities().Supplier.Find(newId) != null)
                    return true;
            }
            return false;
        }
        ...
        {
            CRMEntities db = new CRMEntities();
            Supplier toBeUpdated = db.Supplier.Find(id);
